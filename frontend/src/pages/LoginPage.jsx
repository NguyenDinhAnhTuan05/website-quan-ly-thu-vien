...
export default function LoginPage() {
  ...
  // Show OAuth2 error from redirect
  useEffect(() => {
    if (oauthError) clearError();
  }, [oauthError]);

  const handleOAuth2Login = (provider) => {
    window.location.href = `${API_BASE}/oauth2/authorize/${provider}?redirect_uri=${encodeURIComponent(REDIRECT_URI)}`;
  };
        ...
        <div className="card p-8">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700"
            >
